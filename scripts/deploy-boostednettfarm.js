...
async function main() {
    ...
    const veNETTObj = require(`../deployments/${chainId === 59902 ? 'testnet' : 'mainnet'}/veNETT.json`);
    const veNETTAddr = veNETTObj.veNETT;
    const NETTObj = require(`../deployments/${chainId === 59902 ? 'testnet' : 'mainnet'}/NETT.json`);
    const NETTAddr = NETTObj.NETT;
    const NETTFarmObj = require(`../deployments/${chainId === 59902 ? 'testnet' : 'mainnet'}/NETTFarm.json`)
    const NETTFarmAddr = NETTFarmObj.NETTFarm;
    // TODO
    const MASTER_PID = chainId === 59902 ? 0 : 20;
    ...
    fs.writeFileSync(path.resolve(__dirname, `../deployments/${chainId === 59902 ? 'testnet' : 'mainnet'}/BoostedNETTFarmProxy.json`), JSON.stringify(addresses, null, 4));
}
